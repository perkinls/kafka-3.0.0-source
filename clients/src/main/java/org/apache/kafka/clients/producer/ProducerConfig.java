...
public class ProducerConfig extends AbstractConfig {
    ...
    /*
     * 调用send()后，报告成功或失败的时间上限。这限制了一条记录在发送之前被延迟的总时间、等待代理确认的时间(如果预期)以及允许重新发送失败的时间。
     * 如果遇到不可恢复的错误、重试次数已耗尽或记录被添加到到达较早交付过期截止日期的批处理中，生产者可能会报告未能在此配置之前发送记录。
     */
    ...
    public ProducerConfig(Map<String, Object> props) {
        ...
        // CONFIG 为默认初始化系统的配置，会根据props替换掉默认参数
        super(CONFIG, props);
    }
