...
/**
 * 将要发送一批次的记录（封装MemoryRecordsBuilder，并且有很多控制用的信息及统计信息）
 *
 * This class is not thread safe and external synchronization must be used when modifying it
 * 该类不是线程安全的，修改时必须使用外部同步
 */
public final class ProducerBatch {
    ...
    final TopicPartition topicPartition;// Topic 和分区
    final ProduceRequestResult produceFuture; // 生产者请求占位符
    private final List<Thunk> thunks = new ArrayList<>();//可以理解为消息的回调对象对列，Thunk中的callback字段就指向对应消息的Callback对象
    private final MemoryRecordsBuilder recordsBuilder; // 消息真正累积存放的地方（ProducerBatch-->MemoryRecordsBuilder-->DataOutputStream）
    private final AtomicInteger attempts = new AtomicInteger(0); // //尝试发送当前RecordBatch的次数
    private final boolean isSplitBatch;  //  批次是否切分
    private final AtomicReference<FinalState> finalState = new AtomicReference<>(null); // 最终状态

    int recordCount; // 批次记录数
    int maxRecordSize; // 最大记录大小
    private long lastAttemptMs; // 上一次尝试时间
    private long lastAppendTime; // 上一次添加时间
    ...
    public FutureRecordMetadata tryAppend(long timestamp, byte[] key, byte[] value, Header[] headers, Callback callback, long now) {
        ...
        if (!recordsBuilder.hasRoomFor(timestamp, key, value, headers)) {
            ...
            /* step2.追加消息..MemoryRecordsBuilder调用append方法 */
     ...
     * Thunk指的是消息的回调对象，Thunk中的callback字段就指向对应消息的Callback对象
     */
    ...
    /**
     * 释放记录追加所需的资源（例如压缩缓冲区）。调用此方法后，只能更新 RecordBatch 标头。
