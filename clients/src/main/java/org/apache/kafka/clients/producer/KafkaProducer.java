...
public class KafkaProducer<K, V> implements Producer<K, V> {
    ...
    KafkaProducer(ProducerConfig config,
        ...
        try {
            ...
            this.clientId = config.getString(ProducerConfig.CLIENT_ID_CONFIG); // 获取客户端id
            ...
            /* <Metrics>模块 监控当前clientId相关的一些指标 https://www.freesion.com/article/5844245262/ */
            ...
            this.partitioner = config.getConfiguredInstance(
                    ...
                    Partitioner.class,
                    Collections.singletonMap(ProducerConfig.CLIENT_ID_CONFIG, clientId));
            // retry.backoff.ms 在尝试将一个失败的请求重试到给定的 topic 分区之前需要等待的时间
            ...
            this.maxRequestSize = config.getInt(ProducerConfig.MAX_REQUEST_SIZE_CONFIG); // 单条日志最大大小 max.request.size 默认1m
            this.totalMemorySize = config.getLong(ProducerConfig.BUFFER_MEMORY_CONFIG); //缓冲区大小 buffer.memory 默认32m
            this.compressionType = CompressionType.forName(config.getString(ProducerConfig.COMPRESSION_TYPE_CONFIG)); // 压缩 compression.type默认是none
            this.maxBlockTimeMs = config.getLong(ProducerConfig.MAX_BLOCK_MS_CONFIG);// max.block.ms 最大阻塞时间，由于缓冲区已满或元数据不可用而被阻塞
            int deliveryTimeoutMs = configureDeliveryTimeout(config, log);
            this.apiVersions = new ApiVersions();
            this.transactionManager = configureTransactionState(config, logContext); // 初始化幂等姓类
            ...
            this.accumulator = new RecordAccumulator(
                    ...
                    retryBackoffMs, // 在尝试将一个失败的请求重试到给定的 topic 分区之前需要等待的时间
                    ...
                    new BufferPool(this.totalMemorySize, // 缓冲区对象 默认是32m
                            config.getInt(ProducerConfig.BATCH_SIZE_CONFIG), // 批次大小 batch.size
                            metrics,
                            time,
                            PRODUCER_METRIC_GROUP_NAME));
            ...
            if (metadata != null) {
                ...
                this.metadata = new ProducerMetadata(
                        retryBackoffMs, // 在尝试将一个失败的请求重试到给定的 topic 分区之前需要等待的时间
                        config.getLong(ProducerConfig.METADATA_MAX_AGE_CONFIG), // metadata.max.age.ms 配置元数据的过期时间，默认值为300000(ms)，即5分钟
                        config.getLong(ProducerConfig.METADATA_MAX_IDLE_CONFIG),// metadata.max.idle.ms 控制生产者将为空闲topic缓存元数据的时间 默认5分钟
                        logContext,
                        clusterResourceListeners,
                        Time.SYSTEM);

                // bootstrap方法会创建一个Cluster对象和MetadataCache对象
    ...
    Sender newSender(LogContext logContext, KafkaClient kafkaClient, ProducerMetadata metadata) {
        ...
        // 获取到channel通道 (根据协议security.protocol 来确定)
        ChannelBuilder channelBuilder = ClientUtils.createChannelBuilder(producerConfig, time, logContext);
        ProducerMetrics metricsRegistry = new ProducerMetrics(this.metrics);
        Sensor throttleTimeSensor = Sender.throttleTimeSensor(metricsRegistry.senderMetrics);

        /*
         * 创建一个Kafka客户端对象=>NetworkClient
         */
        KafkaClient client = kafkaClient != null ? kafkaClient : new NetworkClient(
                ...
                metadata,// 生产者元数据
                clientId, // 客户端id
                maxInflightRequests,// 一次拉取客户端数据条数
                producerConfig.getLong(ProducerConfig.RECONNECT_BACKOFF_MS_CONFIG), // 重试时间
                producerConfig.getLong(ProducerConfig.RECONNECT_BACKOFF_MAX_MS_CONFIG),// 重试总时间
                producerConfig.getInt(ProducerConfig.SEND_BUFFER_CONFIG), // 发送缓存区大小
                producerConfig.getInt(ProducerConfig.RECEIVE_BUFFER_CONFIG),// 接收缓存区大小
                requestTimeoutMs, // 请求超时时间
                producerConfig.getLong(ProducerConfig.SOCKET_CONNECTION_SETUP_TIMEOUT_MS_CONFIG), // socket建立超时时间
                producerConfig.getLong(ProducerConfig.SOCKET_CONNECTION_SETUP_TIMEOUT_MAX_MS_CONFIG),// socket建立超时时间最大
                time, // 系统时间
                true,
                apiVersions, // kafka版本
                throttleTimeSensor,// 度量指标
                logContext);
    ...
    private TransactionManager configureTransactionState(ProducerConfig config, LogContext logContext) {
        ...
        if (config.idempotenceEnabled()) { // 多方面确定开启幂等性特性
    ...
    public Future<RecordMetadata> send(ProducerRecord<K, V> record, Callback callback) {
        // TODO 发送消息源代码入口
        // 拦截器相关操作, interceptedRecord代表经过拦截器处理后的数据
    ...
    private Future<RecordMetadata> doSend(ProducerRecord<K, V> record, Callback callback) {
        ...
        try {
            throwIfProducerClosed(); // 判断后台Sender线程是否在线
            /* 首先需要确保元数据中对应的topic是否可用 */
            long nowMs = time.milliseconds();
            ClusterAndWaitTime clusterAndWaitTime;
            //1. 等待集群元数据{clusterInstance,更新元数据的时间}
            try {
            ...
            /* 4. 向accumulator缓存中追加数据。result表示是否添加成功的结果 */
            ...
            if (result.abortForNewBatch) {
                int prevPartition = partition;// 确定先前为触发新批次的记录选择的分区
                partitioner.onNewBatch(record.topic(), cluster, prevPartition); // 新批次产生，重新计算粘性分区
                partition = partition(record, serializedKey, serializedValue, cluster); // 再次获取新的分区值（TODO 为什么会重新计算？）
                ...
                result = accumulator.append(
                        ...
                        headers,            // 记录header
                        interceptCallback,  // 请求完成时，执行的用户提供的回调
                        remainingWaitMs,    // 最大阻塞时间（总共 - 已用）
                        false, // 上面已经添加了一个批次，此处设置为false
                        nowMs);             // 当前时间
            }

            //note: 如何开启了幂等性或事务性，须要作一些处理
            if (transactionManager != null && transactionManager.isTransactional())
            ...
            if (result.batchIsFull || result.newBatchCreated) {
                ...
                /*
                 * >>>> 唤醒线程 -> org.apache.kafka.common.network.Selector#poll select阻塞
                 * 当有新的 RecordBatch 创建后，旧的 RecordBatch 就可以发送了，如果线程阻塞在 select() 方法中，就将其唤醒，
                 * Sender 重新开始运行 run() 方法，在这个方法中，旧的 RecordBatch 将会被选中，进而可以及时将这些请求发送出去。
                 */
                this.sender.wakeup();
            }
     ...
     *
     * @param topic     - 我们想要元数据的主题
     * @param partition – 预期存在于元数据中的特定分区，如果没有首选项，则为 null
     * @param nowMs     – 当前时间（以毫秒为单位）
     * @param maxWaitMs – 等待元数据的最长时间（毫秒）
     * @return 包含主题元数据的集群和我们在毫秒中等待的时间量
     * @throws TimeoutException if metadata could not be refreshed within {@code max.block.ms}
     * @throws KafkaException   for all Kafka-related exceptions, including the case where this method is called after producer close
     */
    private ClusterAndWaitTime waitOnMetadata(String topic, Integer partition, long nowMs, long maxWaitMs) throws InterruptedException {
        // 获取集群对象（KafkaProducer初始化bootstrap会创建Cluster对象）
        Cluster cluster = metadata.fetch();
        if (cluster.invalidTopics().contains(topic)) // 用于判断是否无效的topic
            throw new InvalidTopicException(topic);
        metadata.add(topic, nowMs);// 重置topic过期时间

        Integer partitionsCount = cluster.partitionCountForTopic(topic); // 获取Topic的分区数
        ...
        // 更新过程中已经消耗的时间
        long elapsed = 0;
        // 发出元数据请求，直到我们拥有主题和请求分区的元数据，或者直到超过 maxWaitTimeMs。如果元数据过时并且此主题的分区数量同时增加，这是必要的。
        do {
            if (partition != null) {
            ...
            // 获取上一次更新的version，并将needFullUpdate或needPartialUpdate强制设置为true
            int version = metadata.requestUpdateForTopic(topic); // 获取Topic当前版本

            /*
             * 唤醒sender线程，sender线程又会唤醒NetworkClient线程并发送updateMetadataRequest请求
             */
            sender.wakeup();
            try {
        ...
        } while (partitionsCount == null || (partition != null && partition >= partitionsCount)); // 一直获取更新，直到抛出异常
    ...
    private static class InterceptorCallback<K, V> implements Callback {
        private final Callback userCallback;// 用户回调
        private final ProducerInterceptors<K, V> interceptors;
