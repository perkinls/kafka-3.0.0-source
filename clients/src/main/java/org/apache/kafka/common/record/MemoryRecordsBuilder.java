...
/**
 * 每个ProducerBatch都维护了一个MemoryRecordsBuilder。ProducerBatch追加消息，实际是调用MemoryRecordsBuilder完成的。
 * 消息最终通过MemoryRecordsBuilder的append方法，追加到MemoryRecordsBuilder的DataOutputStream中。
 * <p>
 * 总结一下MemoryRecordsBuilder：它内部维护了一个buffer，并记录能写入的大小，以及写入的位置在哪里，而每条消息都被追加到DataOutput对象appendStream中。
...
public class MemoryRecordsBuilder implements AutoCloseable {
    ...
    /* 用于保存对底层ByteBuffer的引用，以便我们可以写入记录批处理头并访问写入的字节。
     *如果现有的ByteBufferOutputStream不够大，则会分配一个新的ByteBuffer，因此保存对底层ByteBuffer的直接引用是不安全的。
     */
    private final ByteBufferOutputStream bufferStream; // 就是对RecordAccumulator中申请Buffer的包装
    ...
    private Long lastOffset = null; // 用于维护buffer中的偏移量
     ...
     * 消息的追加最终发生在这里:
     * 1.把字节数组形式的key和value转为HeapByteBuffer
     * 2.计算写入的offset，如果不是第一次写入，那么lastOffset+1，否则是baseOffset
     * 3.如果magic号大于1，那么调用appendDefaultRecord(),否则调用appendLegacyRecord()
     */
    private void appendWithOffset(long offset, boolean isControlRecord, long timestamp, ByteBuffer key,
        ...
        try {
            ...
            if (lastOffset != null && offset <= lastOffset) // 判断offset的有效性
                throw new IllegalArgumentException(String.format("Illegal offset %s following previous offset %s " +
                        "(Offsets must increase monotonically).", offset, lastOffset));

            if (timestamp < 0 && timestamp != RecordBatch.NO_TIMESTAMP) // 判断时间戳的有效性
                throw new IllegalArgumentException("Invalid negative timestamp " + timestamp);

            if (magic < RecordBatch.MAGIC_VALUE_V2 && headers != null && headers.length > 0) // 判断kafka版本和header
                throw new IllegalArgumentException("Magic v" + magic + " does not support record headers");

            if (firstTimestamp == null) // 确保消息是首次
                firstTimestamp = timestamp;

            /*
             * 据KafkaProducer客户端版本的不同去调用下面两个方法之一, 追加消息到DataOutputStream
             * {@link https://blog.csdn.net/h2503652646/article/details/112709057}
             */
            if (magic > RecordBatch.MAGIC_VALUE_V1) {
    ...
    /**
     * DataOutputStream里面持有ByteBufferOutputStream，这是一个缓存buffer，所以往DataOutputStream里面写消息数据，
     * 就是往缓存里面写消息数据，后面的recordWritten方法主要是处理位移问题
     */
    private void appendDefaultRecord(long offset, long timestamp, ByteBuffer key, ByteBuffer value,
                                     Header[] headers) throws IOException {
        ensureOpenForRecordAppend(); // 确保 DataOutputStream 打开
        int offsetDelta = (int) (offset - baseOffset); // 偏移量增量
        long timestampDelta = timestamp - firstTimestamp; // 时间戳增量

        /*  数据写入DataStream，返回写入数据的总大小 */
        int sizeInBytes = DefaultRecord.writeTo(appendStream, offsetDelta, timestampDelta, key, value, headers);

        /* 维护MemoryRecordsBuilder的元数据，位移偏移量等 */
        recordWritten(offset, timestamp, sizeInBytes);
    }
